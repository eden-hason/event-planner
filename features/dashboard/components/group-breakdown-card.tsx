...
export function GroupBreakdownCard({ groups }: { groups: GroupWithGuestsApp[] }) {
  ...
  return (
    <Card className={cn('flex h-full flex-col', cardHover)}>
      <CardHeader className="pb-3">
        <CardTitle className="text-sm font-semibold">Guest Groups</CardTitle>
        <p className="mt-0.5 text-xs text-muted-foreground">Breakdown by group</p>
      </CardHeader>
      <CardContent className="flex flex-1 flex-col gap-5 pb-6">
              ...
              <Tooltip key={group.id}>
                ...
                <TooltipContent>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {group.total} {group.total === 1 ? 'guest' : 'guests'} · {pctRounded}%
                  </p>
                </TooltipContent>
              </Tooltip>
            );
        ...
        <div className="space-y-3">
            ...
            return (
              <div key={group.id} className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium leading-none">{group.name}</p>
          ...
          {rows.length === 0 && (
            <p className="text-center text-xs text-muted-foreground">No groups yet</p>
          )}
        </div>
      </CardContent>
    </Card>
