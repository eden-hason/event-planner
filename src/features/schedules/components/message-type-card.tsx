...
interface MessageTypeCardProps {
  ...
  offersNote: boolean;
  /** The Schedule has gone out, so the choice can no longer change. */
  sent: boolean;
}
 ...
 */
export async function MessageTypeCard({ offersNote, sent }: MessageTypeCardProps) {
  const t = await getTranslations('schedules.messageType');

  return (
    <SettingsCard
      title={t('cardTitle')}
      aside={sent ? <ScheduleLockBadge /> : undefined}
    >
        ...
        >
          ...
          <div className="flex min-w-0 flex-col gap-0.5">
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              {offersNote && (
                <span className="bg-violet-tint text-violet-strong rounded-full px-2 py-0.5 text-[10.5px] font-bold">
                  {t('supportsNote')}
                </span>
              )}
            </div>
