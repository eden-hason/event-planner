-- save_the_date_1_no_button: save_the_date_1 with the invitation image and
-- without the "הוספה ליומן" calendar button.
--
-- The same content as save_the_date_1_no_buttons (20260923000000), registered
-- in Meta under its own name, save_the_date_1_image_no_button. The two
-- families are independent; each Event's Schedule points at one of them.
--
...
--
-- Grid: 1 WhatsApp row (save_the_date_1_image_no_button) and 1 SMS row, the
...
values
(
  'save_the_date_1_no_button', 'whatsapp',
  (select id from public.schedule_types where key = 'initial_invitation'),
  '1', 'Save the Date - 1, invitation image, no button',
  ...
  'he', 'save_the_date_1_image_no_button', false, false, false, false, true,
  $json$
{
...
(
  'save_the_date_1_no_button', 'sms',
  (select id from public.schedule_types where key = 'initial_invitation'),
  '1', 'Save the Date - 1, no button',
...
begin
  ...
  where key = 'save_the_date_1_no_button' and channel = 'whatsapp';
  if v_wa_rows <> 1 then
    raise exception 'save_the_date_1_no_button_template: expected 1 WhatsApp row, found %', v_wa_rows;
  end if;

  select count(*) into v_sms_rows
  from public.message_templates
  where key = 'save_the_date_1_no_button' and channel = 'sms';
  if v_sms_rows <> 1 then
    raise exception 'save_the_date_1_no_button_template: expected 1 SMS row, found %', v_sms_rows;
  end if;

  select count(*) into v_bad_placeholders
  from public.message_templates
  where key = 'save_the_date_1_no_button'
  ...
  if v_bad_placeholders > 0 then
    raise exception 'save_the_date_1_no_button_template: % row(s) have the wrong placeholder count', v_bad_placeholders;
  end if;
  ...
  if v_bad_wa > 0 then
    raise exception 'save_the_date_1_no_button_template: the WhatsApp row must have the image header and no buttons';
  end if;
end $$;
